<table id="tabelKu" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
    ...
    <tbody>
        @foreach ($stakir as $item)
        <tr class='clickable-row' data-id='{{ $item->id }}'>
            <td>{{ $loop->iteration }}</td>
            <td>{{ $item->bayar->tujuan->user->name }}</td>
            <td>{{ $item->bayar->tujuan->nm_penerima }}</td>
            <td>{{ $item->bayar->tujuan->kelurahan->kecamatan->nm_kecamatan }}</td>
            <td>{{ $item->bayar->tujuan->kelurahan->nm_kelurahan }}</td>
            <td>{{ $item->bayar->tujuan->alamat }}</td>
            <td>{{ $item->no_resi }}</td>
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#status_kirim').val(data.status_kirim);
            ...
            if (result.value) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#tampil').load('{{ route("pengiriman.create") }}');
                    }
                })
            }
          })
